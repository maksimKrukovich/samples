...
import { LibModels } from "./library/LibModels.sol";
  ...
  ReentrancyGuardUpgradeable
{
  ...
  // Payout Registry address
  address private payoutRegistry;

  // EIP-712 domain separator
  bytes32 public domainSeparator;

  bytes32 internal constant AFFILIATE_TYPE_HASH =
    keccak256("Affiliate(address wallet)");
  ...
  function initializeV2(
    ...
    require(_verificationAddress != address(0), "Zero verification address");
    require(_payoutRegistry != address(0), "Zero Payout Registry address");
    ...
    domainSeparator = keccak256(
      abi.encode(
        ...
        keccak256(bytes("Vitreus Affiliate")),
        keccak256(bytes("1")),
        block.chainid,
        address(this)
      )
    ...
    payoutRegistry = _payoutRegistry;
  ...
  receive() external payable whenNotPaused onlyNotUSDCReceiver {
    ...
    if (balanceOf(msg.sender) > 0) {
      _deposit(
        ...
        _swapETHForUSDC(msg.value),
        address(0)
      );
    } else {
      _mint(msg.sender, _nextTokenId(), _swapETHForUSDC(msg.value), address(0));
    }
  }
   ...
   */
  function mint(
    bytes calldata affiliateSignature,
    address affiliateWallet
  ) external payable whenNotPaused onlyNotUSDCReceiver {
    require(msg.value != 0, "Value must not be zero");

    if (affiliateSignature.length != 0)
      require(
        validate(affiliateWallet, affiliateSignature),
        "Incorrect prover"
      );
  ...
  function validate(
    ...
    bytes32 digest = keccak256(
      abi.encodePacked(
        "\x19\x01",
        domainSeparator,
        keccak256(abi.encode(AFFILIATE_TYPE_HASH, affiliate))
      )
    );

    return digest.recover(signature) == verificationAddress;
  }
  ...
  function mintWithUSDC(
    ...
    bytes calldata affiliateSignature,
    address affiliateWallet
  ) external whenNotPaused onlyNotUSDCReceiver {
    ...
    _mint(to_, _nextTokenId(), value_, affiliateWallet);
    require(
  ...
  function depositUSDC(
    ...
    _deposit(_tokenOwners[msg.sender], amount_, affiliateWallet);
    require(
  ...
  function forceMint(address to_, uint64 value_) external onlyOwner {
    ...
    _deposit(nextToken, value_, address(0));
    _mint(to_, nextToken);
  }
   ...
   */
  function changeUpcomingRound(
    uint256 roundId_,
    uint56 newAllocation_
  ) external onlyOwner {
  ...
  function estimateUSDCAmount(uint256 _amount) external view returns (uint256) {
    (uint256 reserve0, uint256 reserve1, ) = IUniswapV2Pair(_uniswapPair)
      .getReserves();
   ...
   */
  function getPresaleRound(
    uint256 roundId_
  ) external view returns (PresaleRound memory) {
   ...
   */
  function getPresaleRoundAmount() external view returns (uint256) {
   ...
   */
  function getCurrentPresaleRound()
    external
    view
   ...
   */
  function statsOf(uint256 _tokenId) external view returns (uint256[] memory) {
    uint256[] memory roundStatistic = new uint256[](
      _currentPresaleRoundNumber() + 1
    );
  ...
  function approve(
    address to,
    uint256 tokenId
  ) external virtual override onlyAllowedOperatorApproval(to) {
   ...
   */
  function presaleRoundNumber() external view returns (uint256) {
   ...
   * @param tokenId The voucher tokenId
   */
  function availableDeposit(uint256 tokenId) external view returns (uint256) {
    uint256 tokenDeposit = _currentPresaleRound().maximumDeposit -
  ...
  //   function hash(address affiliateWallet) internal pure returns (bytes32) {
  //     return keccak256(abi.encode(MINTER_TYPEHASH, affiliateWallet));
  //   }
  ...
  function _requireDepositAboveMin(uint256 tokenId) internal view {
    require(
      _deposits[tokenId][_currentPresaleRoundNumber()] >=
        _currentPresaleRound().minimumDeposit,
  ...
  function _requireDepositsBelowMax(uint256 tokenId) internal view {
    require(
      _deposits[tokenId][_currentPresaleRoundNumber()] <=
        _currentPresaleRound().maximumDeposit,
  ...
  function _mint(
    ...
    _deposit(tokenId_, value_, affiliateWallet);
   ...
   */
  function _currentPresaleRoundNumber() internal view returns (uint256) {
  ...
  function _deposit(
    ...
    // TODO check that call will be reverted if deposit above min !!! only if signature exists
    (bool success, ) = payoutRegistry.call(
      abi.encodeWithSelector(
        PayoutRegistry.registerPayout.selector,
        affiliateWallet,
        _currentPresaleRoundNumber(),
        value_
      )
    );
    require(success, "Register payout failed");
